...
const GovernanceCard = function ({
  ...
  const mainTitle = (
    <h4 className={tipReason ? 'tipTitle' : ''}>
      <div>{method || tipReason || title || noTitle}</div>
    </h4>
  )
...
export default styled(GovernanceCard)`
  ...
  &.own-proposal {
    ...
    border-left-color: ${({ theme }) => theme.colors.pink_primary};
    padding: calc(2rem - 4px);
  }

  &:hover {
    box-shadow: ${({ theme }) => theme.colors.box_shadow_card_hover};
