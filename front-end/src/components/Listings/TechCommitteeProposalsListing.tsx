...
const TechCommitteeProposals = ({ className, data }: Props) => {
  ...
  if (noPost || !atLeastOneCurrentTechCommitteeProposal)
    return (
      <NothingFoundCard
        className={className}
        text="There are currently no active tech committee proposals."
      />
    )
    ...
    <ul className={`${className} motions__list`}>
      {data.posts.map((post) => {
        ...
        return (
          ...
          !!post.onchain_link?.onchain_tech_committee_proposal.length && (
            <li key={post.id} className="tech_committee_proposals_item">
              {
                <Link to={`/tech/${onchainId}`}>
                  <GovernanceCard
                    ...
                    onchainId={onchainId}
                    status={
                      post.onchain_link.onchain_tech_committee_proposal[0]?.status?.[0].status
                    }
                    title={post.title}
