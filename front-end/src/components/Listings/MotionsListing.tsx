...
const Motions = ({ className, data }: Props) => {
  ...
  if (noPost || !atLeastOneCurrentMotion)
    return <NothingFoundCard className={className} text="There are currently no active motions." />
    ...
    <ul className={`${className} motions__list`}>
      {data.posts.map((post) => {
        ...
        return (
          ...
          !!post.onchain_link?.onchain_motion.length && (
