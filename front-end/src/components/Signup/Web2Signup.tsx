    ...
    <Form className={className} onSubmit={handleSubmit(handleSubmitForm)}>
      ...
      <Form.Group>
        <Form.Field width={16}>
          ...
          {(errors.username as FieldError)?.type === 'maxLength' && (
            <span className={'errorText'}>{messages.VALIDATION_USERNAME_MAXLENGTH_ERROR}</span>
          )}
          {(errors.username as FieldError)?.type === 'minLength' && (
            <span className={'errorText'}>{messages.VALIDATION_USERNAME_MINLENGTH_ERROR}</span>
          )}
          {(errors.username as FieldError)?.type === 'pattern' && (
            <span className={'errorText'}>{messages.VALIDATION_USERNAME_PATTERN_ERROR}</span>
          )}
          {(errors.username as FieldError)?.type === 'required' && (
            <span className={'errorText'}>{messages.VALIDATION_USERNAME_REQUIRED_ERROR}</span>
          )}
        </Form.Field>
      </Form.Group>
      <Form.Group>
        <Form.Field width={16}>
          <label>
            ...
            <HelperTooltip content="Your email is used for password recovery or discussion notifications." />
          </label>
          <input
      ...
      <Form.Group>
        <Form.Field width={16}>
          ...
          />
          {errors.password && (
            <span className={'errorText'}>{messages.VALIDATION_PASSWORD_ERROR}</span>
          )}
        </Form.Field>
      </Form.Group>
      <Form.Field>
        <label className="checkbox-label">
          ...
          <Link to="/terms-and-conditions">Polkassembly end user agreement</Link>.
        </label>
        {errors.termsandconditions && (
          <div className={'errorText'}>
            Please agree to the terms of the Polkassembly end user agreement.
          </div>
        )}
      </Form.Field>
      ...
      <div className={'mainButtonContainer'}>
        <Button primary disabled={loading} onClick={handleSubmitForm} type="submit">
          Sign-up
        </Button>
      ...
      <div className={'mainButtonContainer'}>
        <Button secondary disabled={loading} onClick={handleToggle}>
...
export default styled(SignupForm)`
  ...
  .checkbox-label {
    ...
    color: ${({ theme }) => theme.colors.grey_primary} !important;
    a {
