...
const ReferendumVoteInfo = ({ className, referendumId, threshold }: Props) => {
  ...
  const turnoutPercentage = useMemo(() => {
    ...
    // BN doens't handle floats. If we devide a number by a bigger number (12/100 --> 0.12),
    // the result will be 0 therefore, we first multiply by 10 000,
    // which gives (120 000/100 = 1200) go to Number which supports floats
  ...
  const getThreshold = useMemo(() => {
    ...
    if (isPassing) {
      const res = getFailingThreshold({
        ayes: ayeVotes,
        ayesWithoutConviction: ayeVotesWithoutConviction,
        threshold,
        totalIssuance
      })
      ...
      const res = getPassingThreshold({
        nays: nayVotes,
        naysWithoutConviction: nayVotesWithoutConviction,
        threshold,
        totalIssuance
      })
    ...
    ayeVotes,
    ayeVotesWithoutConviction,
    isPassing,
    nayVotes,
    nayVotesWithoutConviction,
    threshold,
    totalIssuance
  ])
  ...
  useEffect(() => {
    ...
    api.query.democracy
      .referendumInfoOf(referendumId, (info) => {
          ...
          <>
            {isPassing === null ? (
              <Loader
                ...
                text={'Loading vote progress'}
                timeout={30000}
                timeoutText="Vote calculation failed"
              />
            ) : (
              <VoteProgress
                ayeVotes={ayeVotes}
              ...
              <Grid.Row>
                <Grid.Column>
                  <h6>
                    Turnout{' '}
                    {turnoutPercentage > 0 && (
                      <span className="turnoutPercentage">({turnoutPercentage}%)</span>
                    )}
                  </h6>
                ...
                <Grid.Column>
                  <h6>
                    Aye <HelperTooltip content="Aye votes without taking conviction into account" />
                  </h6>
                  <div>
                    {formatBnBalance(ayeVotesWithoutConviction, {
                      numberAfterComma: 2,
                      withUnit: true
                    })}
                  </div>
                ...
                <Grid.Column>
                  <h6>
                    Nay <HelperTooltip content="Nay votes without taking conviction into account" />
                  </h6>
                  <div>
                    {formatBnBalance(nayVotesWithoutConviction, {
                      numberAfterComma: 2,
                      withUnit: true
                    })}
                  </div>
