...
import {
  CouncilAtBlockNumberQuery,
  CouncilAtBlockNumberQueryVariables,
  PollVotesQuery,
  useCouncilAtBlockNumberQuery
...
const CouncilSignals = ({ className, endBlock, data }: Props) => {
  ...
  const councilAtPollEndBlockNumber = useCouncilAtBlockNumberQuery({
    variables: { blockNumber: endBlock }
  })
  const councilAtCurrentBlockNumber = useCouncilAtBlockNumberQuery({
    variables: { blockNumber: currentBlockNumber }
  })

  const getCouncilMembers = (
    councilAtBlockNumber: QueryResult<CouncilAtBlockNumberQuery, CouncilAtBlockNumberQueryVariables>
  ): Set<string> => {
  ...
  return (
    <Card className={className}>
      <h3>
        Council Signals{' '}
        <HelperTooltip content="This represents the off-chain votes of council members" />
      </h3>
      <CouncilSignalBar ayeSignals={ayes} councilSize={memberSet.size} naySignals={nays} />
