...
interface Props {
  ...
  votesRefetch: (
    variables?: PollVotesQueryVariables | undefined
  ) => Promise<ApolloQueryResult<PollVotesQuery>>
}
...
const CouncilSignals = ({
  ...
  return (
    <Card className={className}>
      <h3>
        Poll Signals {/*eslint-disable-next-line max-len*/}
        <HelperTooltip content="This represents the off-chain votes of Polkassembly users including council members" />
      </h3>
      <GeneralChainSignalBar ayeSignals={ayes} naySignals={nays} />
      <div>{error?.message && <FilteredError className="info" text={error.message} />}</div>
        ...
        <div>
          ...
          {canVote ? (
            ...
            <span>
              Poll ended.{' '}
              {canEdit ? (
                <ButtonLink className="info" onClick={extendsPoll}>
                  Extend Poll
                </ButtonLink>
              ) : (
                ''
