...
export default function Discussion({
  ...
  return (
    <DiscussionCard className={ownPost ? 'own-post' : ''}>
      <h4>{title}</h4>
      <CreationLabel created_at={created_at} defaultAddress={defaultAddress} username={username} />
      <ul>
        <li>
          <Icon name="comment" /> {comments} comments
        </li>
        {comments !== 'no' && last_update && (
          <li>
            <Icon name="reply" />
            commented {moment.utc(last_update, 'YYYY-MM-DDTHH:mm:ss.SSS').fromNow()}
          </li>
        )}
      </ul>
    </DiscussionCard>
