...
const CouncilEmoji = () => (
  <span aria-label="council member" className="councilMember" role="img">
    👑
  </span>
)
...
const Profile = ({ className }: Props): JSX.Element => {
  ...
  const onTitleChange = (event: React.ChangeEvent<HTMLInputElement>[]) => {
    setTitle(event[0].currentTarget.value)
    return event[0].currentTarget.value
  }
  const onDescriptionChange = (data: Array<string>) => {
  ...
  const handleSend = async () => {
    ...
    const signMessage = `<Bytes>about::network:${network}|address:${address}|title:${
      title || ''
      ...
      <Grid.Column mobile={16} tablet={16} computer={10} largeScreen={10} widescreen={10}>
        {isEditing ? (
          <Form className={className}>
            ...
            <Controller
              as={<TitleForm errorTitle={errors.title} />}
              control={control}
              name="title"
            ...
            />
            <Controller
              as={<ContentForm errorContent={errors.content} />}
              control={control}
              name="description"
            ...
            <div className={'mainButtonContainer'}>
              <Button primary onClick={handleSubmit(handleSend)} disabled={loading} type="submit">
                {loading ? (
                  <>
                    <Icon name="spinner" />
                    Creating
          ...
          <div className="profile_content">
            {aboutQueryResult?.error ? (
              <FilteredError className="info" text={aboutQueryResult?.error?.message} />
            ) : null}
            {aboutQueryResult?.loading ? (
              ...
              <>
                ...
                {canEdit ? (
                  <div className={'mainButtonContainer'}>
                    <Button primary onClick={handleEdit} disabled={loading} type="submit">
                      {loading ? (
                        <>
                          <Icon name="spinner" />
                          Creating
                  ...
                  <Table.Body>
                    ...
                    {identity?.judgements?.length > 0 && (
                      <Table.Row>
                        ...
                        <Table.Cell className="judgments">
                          <Icon name={iconName} color={color} /> {displayJudgements}
                        </Table.Cell>
                      </Table.Row>
                    )}
                    {identity?.pgp && (
                    ...
                    {identity?.twitter && (
                      <Table.Row>
                        ...
                        <Table.Cell>
                          <a href={`https://twitter.com/${identity.twitter.substring(1)}`}>
                            {identity.twitter}
                          </a>
                        </Table.Cell>
                      </Table.Row>
