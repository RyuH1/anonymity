...
const TechCommitteeProposalsContainer = ({ className }: Props) => {
  ...
  const onchainTechCommitteeProposalIds = Object.keys(trackMap.techCommitteeProposal || {}).map(
    (key) => Number(key)
  )

  const { data, error, refetch } = useTrackerTechCommitteeProposalPostsQuery({
    variables: {
