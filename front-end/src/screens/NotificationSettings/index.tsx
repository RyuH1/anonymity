...
const Settings = ({ className }: Props): JSX.Element => {
  ...
  const [changeNotificationPreferenceMutation, { error }] =
  ...
  const updatePreference = () => {
    changeNotificationPreferenceMutation({
      ...
      .then(({ data }) => {
        if (
          data &&
          data.changeNotificationPreference &&
          data.changeNotificationPreference.message
        ) {
          queueNotification({
            header: 'Success!',
  ...
  const handlePostParticipatedChange = (
      ...
      >
        ...
        {!email_verified && (
          <div className="errorText">
            Please set and verify an email <Link to="/settings">in your settings</Link> to receive
            be able to set notifications.
          </div>
        )}
        <Form standalone={false}>
          <Form.Group>
            <Form.Field>
              <Checkbox
                label="Subscribe to post you participate in"
                disabled={!email_verified}
                checked={!!email_verified && postParticipated}
                toggle
                onChange={handlePostParticipatedChange}
              />
            </Form.Field>
          </Form.Group>
          <Form.Group>
            <Form.Field>
              <Checkbox
                label="Subscribe to post you created"
                disabled={!email_verified}
                checked={!!email_verified && postCreated}
                toggle
                onChange={handlePostCreatedChange}
              />
            </Form.Field>
          </Form.Group>
          <Form.Group>
            <Form.Field>
              <Checkbox
                label="Notified for new proposal in council/motion/referendum"
                disabled={!email_verified}
          ...
          <Form.Group>
            <Form.Field>
              <Checkbox
                label="Notified for your own proposals"
                disabled={!email_verified}
                checked={!!email_verified && ownProposal}
                toggle
                onChange={handleOwnProposalChange}
              />
            </Form.Field>
          </Form.Group>
          {changed ? (
            <Form.Group>
              <Form.Field width={6}>
                ...
                <Button secondary onClick={updatePreference} type="submit">
                  Save
                </Button>
              </Form.Field>
            </Form.Group>
