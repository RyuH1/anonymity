...
const CreatePost = ({ className }: Props): JSX.Element => {
  ...
  const handleSend = () => {
    if (currentUser.id && title && content && selectedTopic) {
      ...
      createPostMutation({
        ...
        .then(({ data }) => {
          if (
            data?.insert_posts?.affected_rows &&
            data?.insert_posts?.affected_rows > 0 &&
            data?.insert_posts?.returning?.length &&
            data?.insert_posts?.returning?.[0]?.id
          ) {
      ...
      console.error(
        'Current userid, title, content or selected topic missing',
        currentUser.id,
        title,
        content,
        selectedTopic
  ...
  const onPollChanged = (event: React.FormEvent<HTMLInputElement>, data: CheckboxProps) => {
    setHasPoll(data.checked || false)
  }
      ...
      <Grid.Column mobile={16} tablet={16} computer={12} largeScreen={10} widescreen={10}>
        <Form className={className}>
          ...
          <Form.Group>
            <Form.Field>
              <Checkbox
                label="Add a poll to this discussion"
                checked={hasPoll}
                toggle
          ...
          <TopicsRadio onTopicSelection={(id: number) => setSetlectedTopic(id)} />
