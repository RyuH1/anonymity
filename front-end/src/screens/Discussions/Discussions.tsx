...
import {
  useDiscussionPostsIdAscQuery,
  useDiscussionPostsIdDescQuery,
...
const DiscussionsContainer = ({ sortBy }: Props) => {
  ...
  let postsQuery:
    | typeof useDiscussionPostsIdDescQuery
    | typeof useDiscussionPostsIdAscQuery
    | typeof useLatestDiscussionPostsQuery

  if (sortBy === sortValues.NEWEST) postsQuery = useDiscussionPostsIdDescQuery
  ...
  const { data, error, loading, refetch } = postsQuery({
