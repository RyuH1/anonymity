...
const VoteProgress = ({ ayeVotes, className, isPassing, nayVotes, threshold }: Props) => {
  ...
  return (
    <div className={className}>
      ...
      <div className="voteNumbers nay">Nay: {bnToStringBalanceDelimitor(nayVotes)}</div>
      <Progress className={isPassing ? 'passing' : ''} percent={ayePercent} />
      {
        // don't show the threshold if it's not been calculated yet
        thresholdPercent > 0 && (
          <div id="passingThreshold" style={{ left: thresholdPercent + '%' }}>
            <hr />
            <div className={thresholdPercent < 50 ? 'threshold-left' : 'threshold-right'}>
              {'>'}
